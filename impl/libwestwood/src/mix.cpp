...
// CRC32 table for TS/RA2 hash
static const uint32_t ts_crc_table[256] = {
...
uint32_t mix_hash_ts(std::string_view filename) {
    ...
    for (char ch : filename) {
        ...
        if (c >= 'A' && c <= 'Z') c += 0x20;
        crc = ts_crc_table[(crc ^ c) & 0xFF] ^ (crc >> 8);
    }
    return crc;
}
...
MixGame mix_detect_game(
    MixFormat format, const std::vector<MixEntry>& entries)
{
    // Check for TS marker
...
// RA encrypted header offsets
constexpr size_t KEY_SOURCE_OFFSET = 4;
constexpr size_t ENCRYPTED_HDR_OFFSET = 84;
constexpr size_t ENCRYPTED_IDX_OFFSET = 92;

// Decrypt RA header and extract file count
static Result<uint16_t> decrypt_ra_header(
    const uint8_t* data, size_t size, Blowfish& bf, uint8_t* dec_header)
{
    if (size < ENCRYPTED_IDX_OFFSET)
        return std::unexpected(make_error(ErrorCode::CorruptHeader, "RA enc"));
    ...
    if (count == 0 || count > MAX_FILE_COUNT)
        return std::unexpected(
            make_error(ErrorCode::CorruptHeader, "RA count"));
    return count;
}

// Reconstruct decrypted index from header and encrypted index
static std::vector<uint8_t> reconstruct_ra_index(
    const uint8_t* dec_header,
    const std::vector<uint8_t>& dec_index,
    size_t raw_size)
{
    std::vector<uint8_t> full_index(raw_size);
    full_index[0] = dec_header[6];
    full_index[1] = dec_header[7];
    if (raw_size > 2)
        std::memcpy(full_index.data() + 2, dec_index.data(), raw_size - 2);
    return full_index;
}
...
constexpr size_t RG_DATA_START = 12;

// Parse RG index entries
static void parse_rg_entries(
    MixReaderImpl& impl, const uint8_t* ptr, uint32_t count)
{
    impl.entries.reserve(count);
    for (uint32_t i = 0; i < count; ++i) {
        MixEntry e;
        e.hash = read_u32(ptr);
        e.offset = read_u32(ptr + 4) + RG_DATA_START;
        e.size = read_u32(ptr + 8);
        impl.hash_to_index[e.hash] = impl.entries.size();
        impl.entries.push_back(std::move(e));
        ptr += INDEX_ENTRY_SIZE;
...
// Parse RG filename table
static void parse_rg_names(MixReaderImpl& impl, const uint8_t* data,
                            uint32_t offset, size_t size) {
    if (offset == 0 || offset >= size) return;
    const uint8_t* ptr = data + offset;
    const uint8_t* end = data + size;

    for (size_t i = 0; i < impl.entries.size() && ptr < end; ++i) {
        uint8_t len = *ptr++;
        if (ptr + len > end) break;
        std::string name(reinterpret_cast<const char*>(ptr), len);
        ptr += len;

        uint32_t crc = mix_hash_ts(name);
        auto it = impl.hash_to_index.find(crc);
        if (it != impl.hash_to_index.end()) {
            impl.entries[it->second].name = name;
            impl.name_to_index[name] = it->second;
        }
    }
}

// Renegade format (MIX1)
static Result<void> parse_rg(
    MixReaderImpl& impl, const uint8_t* data, size_t size)
{
    if (size < 12)
        return std::unexpected(make_error(ErrorCode::CorruptHeader, "RG"));
    ...
    impl.info.has_checksum = false;
    impl.info.file_size = size;
    impl.body_offset = RG_DATA_START;

    if (header_offset + 4 > size)
        return std::unexpected(make_error(ErrorCode::CorruptIndex, "RG hdr"));

    uint32_t file_count = read_u32(data + header_offset);
    if (file_count > MAX_FILE_COUNT)
        return std::unexpected(
            make_error(ErrorCode::CorruptHeader, "RG count"));

    impl.info.file_count = static_cast<uint16_t>(file_count);

    size_t index_start = header_offset + 4;
    if (index_start + file_count * INDEX_ENTRY_SIZE > size)
        return std::unexpected(make_error(ErrorCode::CorruptIndex, "RG idx"));

    parse_rg_entries(impl, data + index_start, file_count);
    parse_rg_names(impl, data, names_offset, size);
    return {};
}

// Parse a single BIG entry (offset, size, null-terminated filename)
static Result<const uint8_t*> parse_big_entry(
    MixReaderImpl& impl, const uint8_t* ptr, const uint8_t* end)
{
    if (ptr + 8 > end)
        return std::unexpected(
            make_error(ErrorCode::CorruptIndex, "BIG entry"));
    ...
    if (ptr >= end)
        return std::unexpected(make_error(ErrorCode::CorruptIndex, "BIG name"));
...
static Result<void> parse_big(
    MixReaderImpl& impl, const uint8_t* data, size_t size)
{
    if (size < 16)
        return std::unexpected(make_error(ErrorCode::CorruptHeader, "BIG"));

    uint32_t magic = read_u32(data);
    impl.info.format = MixFormat::BIG;
    impl.info.game = (magic == BIG4_MAGIC)
        ? MixGame::ZeroHour : MixGame::Generals;
    impl.info.encrypted = false;
    impl.info.has_checksum = false;
    impl.info.file_size = size;
    ...
    impl.info.file_count = static_cast<uint16_t>(std::min(entry_count, 65535u));

    const uint8_t* ptr = data + 16;
    const uint8_t* end = data + 16 + index_size;
    if (end > data + size)
        return std::unexpected(make_error(ErrorCode::CorruptIndex, "BIG idx"));

    impl.entries.reserve(entry_count);
    for (uint32_t i = 0; i < entry_count; ++i) {
        auto r = parse_big_entry(impl, ptr, end);
