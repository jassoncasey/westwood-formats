...
class TestMissingDependencies:
    ...
    def test_shp_missing_palette(self, shp_tool, testdata_shp_files, run, temp_dir):
    ...
    def test_vqa_mp4_missing_ffmpeg(self, vqa_tool, testdata_vqa_files, run, temp_dir, monkeypatch):
...
class TestFilePermissions:
    ...
    def test_unwritable_output_dir(self, aud_tool, testdata_aud_files, run, temp_dir):
...
class TestOverwriteProtection:
    ...
    def test_refuse_overwrite_without_force(self, aud_tool, testdata_aud_files, run, temp_dir):
        ...
        result = run(aud_tool, "export", testdata_aud_files[0], "-o", str(existing))
        # Should fail or warn
        if result.returncode == 0:
            # Check file wasn't changed (some implementations may still succeed)
            pass
        else:
            assert "exist" in result.stderr_text.lower() or "force" in result.stderr_text.lower()

    def test_allow_overwrite_with_force(self, aud_tool, testdata_aud_files, run, temp_dir):
        ...
        result = run(aud_tool, "export", "--force", testdata_aud_files[0], "-o", str(existing))
        if result.returncode == 0:
            # File should be different now
            assert existing.stat().st_size != original_size or existing.read_bytes() != b"original content"
...
class TestErrorMessages:
    ...
    def test_nonexistent_file_message(self, aud_tool, run):
        ...
        assert "not found" in result.stderr_text.lower() or "no such" in result.stderr_text.lower() or \
               "nonexistent" in result.stderr_text.lower()
...
class TestPartialProcessing:
    ...
    def test_batch_continues_after_error(self, aud_tool, testdata_aud_files, temp_file, run, temp_dir):
    ...
    def test_output_on_decode_error(self, vqa_tool, testdata_vqa_files, testdata_pal_files, run, temp_dir):
        """Test partial output on decode error."""
        # This tests if tool can produce partial output when decode fails mid-stream
