...
class TestGifSignature:
    ...
    def test_gif_signature_89a(self, wsa_tool, testdata_wsa_files, run, temp_dir):
...
class TestGifDimensions:
    ...
    def test_logical_screen_width(self, wsa_tool, testdata_wsa_files, run, temp_dir):
    ...
    def test_logical_screen_height(self, wsa_tool, testdata_wsa_files, run, temp_dir):
...
class TestGifFrameTiming:
    ...
    def test_custom_fps_option(self, wsa_tool, testdata_wsa_files, run, temp_dir):
        """Test --fps option affects timing."""
        if not testdata_wsa_files:
            pytest.skip("No WSA files in testdata")
        out_file = temp_dir / "test.gif"
        result = run(wsa_tool, "export", "--fps", "10", testdata_wsa_files[0], "-o", str(out_file))
        if result.returncode != 0:
            pytest.skip("Export not implemented")
...
class TestGifLooping:
    ...
    def test_netscape_extension_present(self, wsa_tool, testdata_wsa_files, run, temp_dir):
    ...
    def test_loop_infinite_default(self, wsa_tool, testdata_wsa_files, run, temp_dir):
        """Test default is infinite loop (count=0)."""
        if not testdata_wsa_files:
            pytest.skip("No WSA files in testdata")
        out_file = temp_dir / "test.gif"
        result = run(wsa_tool, "export", "--loop", testdata_wsa_files[0], "-o", str(out_file))
        if result.returncode != 0:
            pytest.skip("Export not implemented")
        # Loop count 0 = infinite
        assert out_file.exists()

    def test_no_loop_option(self, wsa_tool, testdata_wsa_files, run, temp_dir):
        """Test --no-loop produces single-play GIF."""
        if not testdata_wsa_files:
            pytest.skip("No WSA files in testdata")
        out_file = temp_dir / "test.gif"
        result = run(wsa_tool, "export", "--no-loop", testdata_wsa_files[0], "-o", str(out_file))
        if result.returncode != 0:
            pytest.skip("Export not implemented")
...
class TestGifTransparency:
    ...
    def test_transparent_flag_option(self, shp_tool, testdata_shp_files, run, temp_dir):
        """Test --transparent enables transparency."""
        if not testdata_shp_files:
            pytest.skip("No SHP files in testdata")
        out_file = temp_dir / "test.gif"
        result = run(shp_tool, "export", "--transparent", testdata_shp_files[0], "-o", str(out_file))
        if result.returncode != 0:
            pytest.skip("Export not implemented")
        assert out_file.exists()
...
class TestGifColorTable:
    ...
    def test_global_color_table_present(self, wsa_tool, testdata_wsa_files, run, temp_dir):
    ...
    def test_color_table_size(self, wsa_tool, testdata_wsa_files, run, temp_dir):
...
class TestGifAnimation:
    ...
    def test_multiple_frames(self, wsa_tool, testdata_wsa_files, run, temp_dir):
    ...
    def test_graphic_control_extension(self, wsa_tool, testdata_wsa_files, run, temp_dir):
    ...
    def test_trailer_present(self, wsa_tool, testdata_wsa_files, run, temp_dir):
